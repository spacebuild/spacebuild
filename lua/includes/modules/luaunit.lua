...
function table.tostring( tbl, indentLevel, printTableRefs, recursionTable )
    ...
    for k, v in ipairs( tbl ) do
        if recursionTable[v] then
            ...
            table.insert( result, private.prettystr_sub( v, indentLevel+1, false, printTableRefs, recursionTable ) )
        end
    ...
    for k, v in sortedPairs( tbl ) do
        if not done[ k ] then
            if recursionTable[v] then
                ...
                table.insert( result,
                    table.keytostring( k ) .. "=" .. private.prettystr_sub( v, indentLevel+1, true, printTableRefs, recursionTable ) )
            end
        end
    end
...
local function prettystr( v, keeponeline )
    ...
    * tables are expanded
    ...
    local s = private.prettystr_sub(v, 1, keeponeline, PRINT_TABLE_REF_IN_ERROR_MSG, recursionTable)
    if recursionTable['recursionDetected'] == true and PRINT_TABLE_REF_IN_ERROR_MSG == false then
        -- some table contain recursive references, 
        -- so we must recompute the value by including all table references
        -- else the result looks like crap
        recursionTable = {}
        s = private.prettystr_sub(v, 1, keeponeline, true, recursionTable)
    end
    return s
end
prettystr = prettystr
...
private.prettystr_sub = prettystr_sub
...
local TextOutput_MT = { -- class
    ...
    function TextOutput:startTest(testName)
        if self.verbosity > VERBOSITY_DEFAULT then
            Msg( "    ".. self.result.currentNode.testName.." ... " )
        end 
    end 
