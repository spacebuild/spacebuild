...
local colors = {}
colors.red = Color(230, 0, 0, 230)
colors.green = Color(0, 230, 0, 230)
...
function CAFEnts.MakeEnt(tool, ply, Ang, Pos, class, type, sub_type, model, frozen, Extra_Data, Data)
    ...
    if tool.AdminOnly and not ply:IsAdmin() then
        CAF.POPUP(ply, CAF.GetLangVar("caf_stool_admin_required"), "right", colors.red, 0.5);
        return false
    end
    ...
    if tool.EnableFunc then
        if not tool.EnableFunc(ply) then
            CAF.POPUP(ply, CAF.GetLangVar("caf_stool_disabled"), "right", colors.red, 0.5);
            return
        end
    end
    ...
    if devinfo.group.AdminOnly and not ply:IsAdmin() then
        CAF.POPUP(ply, CAF.GetLangVar("caf_stool_entity_admin_required"), "right", colors.red, 0.5);
        return false;
    end
    if devinfo.AdminOnly and not ply:IsAdmin() then
        CAF.POPUP(ply, CAF.GetLangVar("caf_stool_entity_model_admin_required"), "right", colors.red, 0.5);
        return false
    end
    ...
    if devinfo.group.EnableFunc then
        if not devinfo.group.EnableFunc(ply) then
            CAF.POPUP(ply, CAF.GetLangVar("caf_stool_entity_disabled"), "right", colors.red, 0.5);
            return
        end
    end
    if devinfo.EnableFunc then
        if not devinfo.EnableFunc(ply) then
            CAF.POPUP(ply, CAF.GetLangVar("caf_stool_entity_model_disabled"), "right", colors.red, 0.5);
            return
        end
    end
