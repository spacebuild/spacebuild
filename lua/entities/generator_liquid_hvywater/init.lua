...
function ENT:Proc_Water()
    ...
    if not (WireAddon == nil) then Wire_TriggerOutput(self, "WaterUsage", math.Round(winc)) end
    if (energy >= einc and water >= winc) then
        ...
        if not (WireAddon == nil) then Wire_TriggerOutput(self, "HeavyWaterProduction", (math.ceil(HW_Increment * self:GetMultiplier()) * self.Multiplier) * (1 + (self.overdrive * 2))) end
        --end
    else
        self:TurnOff()
    end
