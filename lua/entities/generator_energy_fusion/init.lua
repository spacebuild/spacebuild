		...
		for key, found in pairs(ents.FindInSphere(Pos, blastradius)) do
			...
			if (movetype == 2 or movetype == 3 or movetype == 5 or movetype == 6 or movetype == 8 or movetype == 9) then
				...
				elseif found:IsValid() then
					...
					if (found:GetMoveType() ~= 6) or not physobj:IsValid() then
						found:SetVelocity(vecang * (170 * mass)) --push it away
					elseif (string.find(found:GetClass(), "ragdoll") ~= nil) then
						--if it's a ragdoll...
...
function ENT:Extract_Energy()
	...
	if (self:GetResourceAmount("water") < math.ceil(Coolant_Increment * self:GetMultiplier())) then
		...
		if (inc > 0) then
			inc = math.ceil(inc / math.random(12 - math.ceil(8 * (self:GetResourceAmount("water") / math.ceil(Coolant_Increment * self:GetMultiplier()))), 20))
		end
	else
	...
	if WireLib then
		WireLib.TriggerOutput(self, "Output", inc)
	end
	--[[   	     Base: 2000
...
--leak cause this is like with storage, make be it could leak radation?
function ENT:Leak()
