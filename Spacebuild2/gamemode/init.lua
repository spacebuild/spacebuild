...
AddCSLuaFile( "LS2/cl_lifesupport.lua" )

AddCSLuaFile( "RD2/manifest.lua" )
AddCSLuaFile( "RD2/cl_tab.lua" )
AddCSLuaFile( "RD2/RD2_Tools.lua" )
...
include( "RDBeam_NetVars.lua" )

include( "RD2/manifest.lua" )

include( "LS2/sv_effects.lua" )
include( "LS2/sv_lifesupport.lua" )
		...
		if not ent.planet or not (ent.planet == num) or ent.reset then
			if valid then
				...
				if ent.environment then
					...
					ent.environment.gravity = gravity
					ent.environment.n = n
					if planet then
...
function GM:AddSentsToList()
	...
	for _, item in pairs( SEntList ) do
		local name =  item.t.ClassName or item.t.Classname
		local found = 0
